...
class Inference:
    def __init__(self,
        ...
        if from_checkpoint is True and from_hub is False:
            self.model = AutoModelForCausalLM.from_pretrained(model_checkpoint, device_map = {"": torch.device(f"cuda:{gpu_id}")}).half()
            ...
            self.model = AutoModelForCausalLM.from_pretrained(config.base_model_name_or_path, device_map = {"": torch.device(f"cuda:{gpu_id}")}).half()
